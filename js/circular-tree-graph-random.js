...
function findClosest(point, arrayOfPoints) {
    ...
    let plausibleDists = distances.filter(dist => dist <= minDist + closestSearchThreshold)
    let randomPlausibleDist = plausibleDists[Math.floor(Math.random() * plausibleDists.length)]
    return arrayOfPoints[distances.indexOf(randomPlausibleDist)]
}
